...
class GPT2Classifier:
    ...
    def __init__(self,path,
                 ...
                 val_loader=None, 
                 test_loader=None, 
                 number_of_transfomers_to_be_trained_from_end=0, 
                 num_classes=2, 
                 subfolder_name='tobeknown', 
                 retain_ratio=1,
                 insert_every=12):
        ...
        self.gpt2_fourrier_model = gpt2.initialise_fourrier(retain_ratio=retain_ratio, insert_every=insert_every)
        
        self.BASE_CONFIG = gpt2.configuaration()
        self.transfomer_layers_to_be_trained = number_of_transfomers_to_be_trained_from_end
        self.num_classes = num_classes


        self.BASE_CONFIG['qkv_bias'] = True 
        self.BASE_CONFIG['num_classes'] = num_classes
        self.BASE_CONFIG['transfomer_layers_trained_from_end'] = number_of_transfomers_to_be_trained_from_end


        self.tokenizer = tiktoken.get_encoding("gpt2")
        self.destination_path = path
        self.save_info.saving_model_configurataion(self.destination_path,self.BASE_CONFIG,name_subfolder=subfolder_name)
    ...
    def freeze_fourrier_weights(self):
        for param in self.gpt2_fourrier_model.parameters():
            param.requires_grad = False
    ...
    def fourier_classifier_head(self, number_of_transfomers_to_be_trained_from_end=1 ,num_classes=2):
        torch.manual_seed(123)
        self.gpt2_fourrier_model.out_head = torch.nn.Linear(in_features=self.BASE_CONFIG["emb_dim"], out_features=num_classes)

        for param in self.gpt2_fourrier_model.trf_blocks[number_of_transfomers_to_be_trained_from_end*-1].parameters():
            param.requires_grad = True

        for param in self.gpt2_fourrier_model.final_norm.parameters():
            param.requires_grad = True
    ...
    #Check accuracy with data loaders
    def check_with_loader_fourier(self):

        model = self.gpt2_fourrier_model
        device = self.device
        #print("This is where it is failing\n")
        train_accuracy = self.evaluate.calc_accuracy_loader(self.train_loader, model, device)
        #print("Train accuracy \n")
    ...
    def test_fourrier(self, quesition="Do you have time"):
        ...
        with torch.no_grad():
            outputs = self.gpt2_fourrier_model(inputs)
    ...
    def evaluate_model_fourrier(self, train_loader, val_loader, device, eval_iter):
        model = self.gpt2_fourrier_model
        model.eval()
        with torch.no_grad():
            train_loss = self.evaluate.calc_loss_loader(train_loader, model, device, num_batches=eval_iter)
    ...
    def train_classifier_simple_fourrier(self, 
                                train_loader, 
                                val_loader, 
                                optimizer, 
                                device, 
        ...
        for epoch in range(num_epochs):
            ...
            for input_batch, target_batch in train_loader:
                ...
                # Optional evaluation step
                if global_step % eval_freq == 0:
                    train_loss, val_loss = self.evaluate_model_fourrier(
                         train_loader, val_loader, device, eval_iter)
                    train_losses.append(train_loss)
                    val_losses.append(val_loss)
                    print(f"Ep {epoch+1} (Step {global_step:06d}): "
            ...
            val_accs.append(val_accuracy)
            #save the model state and weights after each and every epoch
            self.save_info.saving_weights_of_model_optimizer(model,optimizer,epoch,self.destination_path,name_subfolder='finetuned_weights_fourrier')
    ...
    def finetune_fourrier(self, epochs):
        
        model = self.gpt2_fourrier_model
        start_time = time.time()
        torch.manual_seed(123)
        optimizer = torch.optim.AdamW(model.parameters(), lr=5e-5, weight_decay=0.1)    
        train_losses, val_losses, train_accs, val_accs, examples_seen = self.train_classifier_simple_fourrier(
